...
for (let i = 1; i <= 10; i++) {
  document.getElementById("btn" + [i]).addEventListener("click", function () {
    ...
    titleChnage.querySelector(".modal-title").textContent =
      sender + " is sending money";
  });
}
...
document.getElementById("sendButton").addEventListener("click", function () {
  ...
  if (senderBalance - amount < 2000) {
    ...
    receiverAmount =
      Number(document.getElementById(receiverName).innerHTML) + Number(amount);
    ...
    var tagContent = document.createTextNode(
      `${sender} sent $${amount} to ${receiverName} on ${Date()}`
    );
